import './style.css';
import {ProcessJoin, ProcessCut, ProcessBoomerang, ProbeVideo, SelectFile, DeleteFile, SaveFileAs, SaveTemp} from '../wailsjs/go/main/App';
...
function render() {
  ...
  app.innerHTML = `
    <header class="tabs">
      ...
      <button class="tab-btn ${state.activeTab === 'compare' ? 'active' : ''}" onclick="window.switchTab('compare')">Compare (6X)</button>
    </header>

    <main class="content">
      <!-- ROW 1: PLAYBACK STAGE -->
      <div id="video-workspace" style="display: grid; grid-template-columns: ${state.activeTab === 'compare' ? 'repeat(2, 1fr)' : 'repeat(auto-fit, minmax(380px, 1fr))'}; gap: 1.5rem; align-items: start; margin-bottom: 2rem;">
        ${renderTabInputs()}
        ${(state.output && state.activeTab !== 'compare') ? window.renderVideoPlayer(state.output, state.output.split('\\').pop(), 'result') : ''}
      </div>

      <!-- ROW 2: FULL-WIDTH CONTROLS -->
      <div id="settings-workspace" style="width: 100%; margin-bottom: 2rem;">
        ${renderTabSettings()}
      </div>
...
function renderTabInputs() {
  ...
  const icon = state.activeTab === 'boomerang' ? '🪃' : '✂️';
  return `
    ${state.video1.path ? 
